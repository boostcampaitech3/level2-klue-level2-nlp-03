...
## RBERT 훈련용입니다.
## 혹시몰라서 파일을 따로 빼두었습니다.
## 수정 & 소연이 먼저 검증으로 사용할 예정입니다.
## [TODO] 해당 모델로 inference 수행시 inference에 아래 부분을 추가하고 모델을 바꿔야합니다.
...
from transformers import AutoTokenizer, AutoConfig, EarlyStoppingCallback, AutoModelForSequenceClassification, Trainer, \
...
def compute_metrics(pred):
    ...
    ### RBERT 확인을 위해 confusion matrix 추가
    ...
    def run(self, args, exp_full_name, reports='wandb'):
        ...
        for fold, (train_idx, val_idx) in enumerate(kfold.split(total_train_dataset, total_train_label)):
            ...
            torch.save(model.state_dict(),
                       os.path.join(f'{args.model_save_dir}_{fold}/{folder_name}', 'pytorch_model.bin'))
