...
def main(args):
  ...
  ## load test datset
  test_dataset_dir = "/opt/ml/git/level2-klue-level2-nlp-03/eunki/dataset/test/test_data.csv"
  ...
  output.to_csv(f'./prediction/{args.model_name}/submission_{fold_num}_moredense8~_best.csv', index=False) # 최종적으로 완성된 예측한 라벨 csv 파일 형태로 저장.
  #### 필수!! ##############################################
  print('---- Finish! ----')
...
if __name__ == '__main__':    
  ...
  for i in range(8):
    ...
    # entity_marker
    # added by sujeong;
    parser.add_argument("--add_entity_marker", type=str2bool, default=True, help="If you want to add entity marker, set this argument True.")
    parser.add_argument("--entity_marker_type", type=str, default="typed_entity_marker_punc",
    ...
    # added by sujeong;
    parser.add_argument("--data_preprocessing", type=str2bool, default=True, help="If you want to make data preprocessed, set this argument True.")
    
    args = parser.parse_args()
    parser.add_argument('--model_dir', type=str, default=f"./best_model_{i}/{args.model_name}/pytorch_model.bin")
    #parser.add_argument('--model_dir', type=str, default=f"./results/fold_{i}/checkpoint-4200/pytorch_model.bin")
