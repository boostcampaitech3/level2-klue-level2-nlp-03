...
class RobertaClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.dense1 = nn.Linear(config.hidden_size, 8*config.hidden_size)
        classifier_dropout = (
            config.classifier_dropout if config.classifier_dropout is not None else config.hidden_dropout_prob
        )
        self.dense2 = nn.Linear(8*config.hidden_size, 4*config.hidden_size)
        self.dense3 = nn.Linear(4*config.hidden_size, 2*config.hidden_size)
...
class RobertaClassificationBidirectionalLSTMHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc1 = nn.Linear(2* config.hidden_size, 4*config.hidden_size)
        self.fc2 = nn.Linear(4* config.hidden_size, config.hidden_size)
        self.fc3 = nn.Linear(config.hidden_size, config.num_labels)
